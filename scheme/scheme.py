...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        formals_point = formals
        vals_point = vals
        while formals_point != nil and vals_point != nil:
            child_frame.define(formals_point.first, vals_point.first)
            formals_point= formals_point.rest
            vals_point = vals_point.rest
        return child_frame
        "*** YOUR CODE HERE ***"
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        new_expressions = Pair(formal,body)
        env.bindings[name] = do_lambda_form(Pair(formal, body), env)
        return name
...
def do_and_form(expressions, env):
    ...
    if point is nil:
        return True
    while point !=nil:
        value = scheme_eval(point.first, env)
...
def make_let_frame(bindings, env):
    ...
    while binding_pointer is not nil:
        ...
        name= binding_pointer.first.first
        tem_value = scheme_eval(binding_pointer.first.rest.first, env)
        names = Pair(name, names)
        values = Pair(tem_value, values)
