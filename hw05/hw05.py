class VendingMachine:
    ...
    def add_funds(self, number):
        if self.inventory == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(number)
        self.fund+=number
...
def store_digits(n):
    ...
    if n < 10:
        ...
        while temp//10>0:
            ...
            num+=1
        first = n//(10**(num-1))
        return Link(first,store_digits(n%(10**(num-1))))
...
def is_bst(t):
    ...
    def bst_min(t):
        if t.is_leaf():
            return t.label
        minvalue_list = [t.label]
        for b in t.branches:
            minvalue_list+=[bst_min(b)]            
        return min(minvalue_list)
    
    def bst_max(t):
        if t.is_leaf():
            return t.label
        maxvalue_list = [t.label]
        for b in t.branches:
            maxvalue_list+=[bst_max(b)]
        return max(maxvalue_list)
