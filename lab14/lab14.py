...
def num_splits(s, d):
    ...
    for i in range(len(s)):
        for j in range(len(split_list)):
            with_new_number_list = [split_list[j]+[s[i]]]
            without_new_number_list = [split_list[j]]
            tmp_list += with_new_number_list + without_new_number_list
        split_list = tmp_list
        tmp_list = []
    sum_list = sum(s)
    lower = (sum_list - d) / 2
    ...
    for m in range(len(split_list)):
        if lower <= sum(split_list[m]) <= upper:
            num+=1
...
def filterl(lst, pred):
    ...
    if lst is link.empty:
        return lst
    return foldr(lst, lambda x, y: Link(x, filter(y,pred)) if pred(x) else filter(y, pred), Link.empty)
