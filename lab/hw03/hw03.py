...
def replace_leaf(t, find_value, replace_value):
    ...
    if label(t) == find_value and is_leaf(t):
        return tree(replace_value,[replace_leaf(b, find_value, replace_value) for b in branches(t)])
    else:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not (lower_bound(y)<=0 and upper_bound(y) >=0), "no zero y"
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    exp_point = -b/(2*a)
    left_interval= a*lower_bound(x)**2 + b*lower_bound(x) + c
    right_interval = a*upper_bound(x)**2 + b*upper_bound(x) + c
    if lower_bound(x)<=exp_point<=upper_bound(x):
        exp_value= a*exp_point**2 + b*exp_point + c
    else:
        exp_value = left_interval

    lower = min(left_interval,right_interval,exp_value)
    upper = max(left_interval,right_interval,exp_value)
    return interval(lower,upper)
