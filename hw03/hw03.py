...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def pingpong_run(around_num, index, count_upordown, pingpong_value):
        if around_num == 1:
            return pingpong_value
        if index%8 == 0 or num_eights(index)> 0:
            return pingpong_run(around_num-1, index+1, -count_upordown, pingpong_value-count_upordown)
        else:
            return pingpong_run(around_num-1, index+1, count_upordown, pingpong_value+count_upordown)
    return pingpong_run(n, 1, 1, 1)
...
def count_coins(change):
    ...
    def help_count(n,m):
        if n<m:
            ...
            return help_count(n-m,m)
        else:
            return help_count(n-m,m)+help_count(n,get_next_coin(m))

    return help_count(change,1)
